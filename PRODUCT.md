...
The separately approved `/notes` index uses a Routed Signal Map for the currently public notes linked to real projects. The 2026-09-07 content revision has 15 public notes across five channels and 12 suspended notes awaiting their own rewrites; the site has eight project homepages. Six approved Smart Car, counter and DIY notes replace three old narratives. Superseded review copies have been removed, with previous versions retained by Git. Visitors can search bilingual metadata and filter by project or frontmatter year before opening a semantic note row; dates remain archive metadata, route motion never claims telemetry or progress, and every `/notes/[slug]` detail stays on the paper reading surface.
