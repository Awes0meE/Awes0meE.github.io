# Active Work: Homepage Release and Arduino Car Learning Notes

**Updated:** 2026-09-06

**Branch:** `docs/rewrite-arduino-power-speed-note`

**State:** Homepage release completed through [PR #36](https://github.com/Awes0meE/Awes0meE.github.io/pull/36), merged as `8e009efd2a00ef8fdbb2a47eaed9bf5b2251020d`. Vercel production succeeded and all 31 production copy/visibility checks passed. The user approved the first combined Arduino power/speed note, then selected black-line sensing and 74HC165 input acquisition together for a second note. The user has now approved the second note as well. Both bilingual notes are final locally; their sources remain private.
...
1. Both notes have content approval. The second, **Arduino 小车的循迹信号与输入采集 / Arduino Car Line Sensing and Input Acquisition**, is available at `http://127.0.0.1:8768/preview.html`. Its source is `content/notes/arduino-smart-car-line-sensing-input-acquisition.mdx`; drafts and checks are in `docs/experiments/arduino-line-sensing-note-2026-09-06/`.
2. The user has approved **Arduino 小车的供电与轮速测量 / Arduino Car Power and Wheel-Speed Measurement**. Its unchanged source and review page remain available at `http://127.0.0.1:8767/preview.html`; the original suspended body is preserved in its packet's `before.mdx` and `8e009ef`.
3. The user has approved both notes; do not request the same content approval again. Both sources remain private until publication work is requested. No next project has been selected. Reuse the confirmed Arduino brief and approved topics.
4. Before the next relevant publication, reconcile the protected homepage's old six-line-sensor count. The manual BOM, §5.4, IR1–IR7 schematic, and seven decoded IR fields agree on seven, including the centre sensor. The second note uses seven. This draft leaves homepage copy untouched and reports the discrepancy rather than claiming a fully reconciled family.
5. Homepage release branches `docs/refactor-project-homepages` and `feat/project-note-division` were removed after their commits were merged. The last verified `main` and `origin/main` are at `8e009ef`; preserve remote `gh-pages` for the GitHub Pages redirect.

## Verification and History

See `../experiments/project-homepages-2026-09-06/verification.json` for homepage pre-release checks, `production-verification.json` for the 31 successful production checks, and the Arduino note packet for draft review. Expected local counts are 8 projects and 25 note sources: 9 public, 15 previously suspended, and 1 new private draft. There are two note-index channels. The deployed homepage-release checkpoint still has 24 note sources. A production build should prerender 24 pages. Older 39-page results describe the previous all-public note set.
