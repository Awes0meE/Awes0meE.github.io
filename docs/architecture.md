...
Current repository interface and content state as of `2026-09-06`, built on the released `v0.9.0` frontend:
...
- 24 note source files total: 9 public and 15 suspended in the 2026-09-06 local candidate; the family counts below include suspended sources;
...
- Historical authoring context for the family inventory below: seven homepages have now been rewritten locally; 15 overlapping notes are suspended, with all note bodies and media unchanged. See `active-work/portfolio-copy-rewrite.md`.
- the DIY pressure-flow cooling project, note, and six media records previously used the approved first-person account from the one-project-at-a-time interview workflow, while remembered temperature and frame-rate changes stay marked as personal observations rather than controlled benchmark results;
...
- the Claude Chime project and cold-start note now use the approved bilingual first-person account; direct no-load and analog-path checks remain separate from client-reported real-solenoid integration, while the protected-pack output collapse supports only a startup-protection inference without a measured inrush waveform or exact state transition;
- the Juanyun thermal project now presents BaseUnit and the three-board ACUnit as one phase-change laptop-cooling system across its original controller work and later automatic-control/maintenance phase; its nine related source notes contain hardware, bare-metal scheduling, refrigeration control, and ControlPanel material; five remain public and four are suspended, while company-reported test results remain explicitly unverified by Alvin;
