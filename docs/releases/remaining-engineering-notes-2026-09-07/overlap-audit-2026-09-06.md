...
This 2026-09-06 audit is retained as historical rationale. All sixteen replacement studies are now approved; the twelve remaining suspended sources have been retired. The recommendations below are not pending tasks. See [the release record](README.md).

# 项目主页与学习笔记重合审阅

2026-09-06。范围为 8 个项目、按 `projectSlug` 关联的 24 篇公开笔记，共 32 页。Claude Chime 以本次获批新版为基准，其他页面以 `a22072a` 中的内容为基准。这是最初的审查快照。用户随后明确批准执行：七个主页已在本地重构，Claude Chime 主页仅维护暂挂链接；15 篇笔记暂挂、9 篇保留。当前清单与恢复说明见 [执行记录](../../experiments/project-homepages-2026-09-06/README.md)，新选题见 [候选题目](topic-decisions.md)。下文保留原始审查意见，不作为当前执行状态。
...
当前[冷启动笔记](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/claude-chime-cold-start-battery-protection.mdx)分为“冷启动排查”“双路升压”“电源路径”“台架验证”。建议保持一篇独立完整的排查文章，按以下内容取舍：
...
| --- | --- | --- | --- |
| Claude Chime | [电池冷启动排查](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/claude-chime-cold-start-battery-protection.mdx) | 测阻、波形、保护机制推断、启动实验 | 按上文集中主题 |
...
| 南京 | [三周开发记录](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/turing-three-week-development-log.mdx) | 角色状态或运行依赖等可单独展开的问题，以及直接相关的失败和修正 | 先确定研究问题，避免重新讲完整三周经历；工具命令转引原文记录 |
...
| 天津 | [环境搭建](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/tianjin-stm32-environment-setup.mdx) | 编译、下载、执行三阶段和最小输出验证 | 主体保留，主页缩短配置与引脚细节 |
| 天津 | [GPIO/EXTI/Timer](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/tianjin-stm32-gpio-exti-timer.mdx) | 电平、边沿、计数、时钟与事件消费 | 缩短开头工具链回顾和编码器消费示例 |
| 天津 | [PWM/UART/ADC](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/tianjin-stm32-pwm-uart-adc.mdx) | 16 点标定、插值、比较寄存器、数据路径 | 作为 16 点映射的详细出处 |
| 天津 | [综合 main()](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/tianjin-metro-environment-monitoring-stm32.mdx) | 快慢变量、flag、模式、共享状态与阻塞限制 | 查表算法只保留调用关系，链接详细笔记 |
| 天津 | [PID/轨道控制阅读](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/tianjin-rail-control-pid-atc-reading.mdx) | 限幅与反馈的区别、PID 到 ATP/ATO/ATS 的尺度变化 | 16 点查表只用作短例，避免再次完整解释 |
| 卷云 | [BaseUnit 固件](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/juanyun-baseunit-firmware.mdx) | UI 状态、按键 latch、DMA、Flash 日志 | 保留，缩短与主页相同的起点及结尾 |
| 卷云 | [ACUnit 三板 bring-up](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/juanyun-acunit-board.mdx) | 电压/电流、逐外设验证、排线问题 | 保留，测量明细由此页承担 |
| 卷云 | [ACUnit 裸机固件](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/juanyun-acunit-firmware.mdx) | 五档节拍、deadline、非阻塞回零与慢任务避让 | 保留完整推理，主页保留选择理由即可 |
| 卷云 | [自动制冷控制](../../../content/notes/juanyun-acunit-automatic-control.mdx) | 双周期、过热度、安全仲裁、启动与标定 | 保留，后期实现/测试边界必须留在附近 |
| 卷云 | [ControlPanel 与维护](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/juanyun-production-line-control-panel.mdx) | 可观测性、控制权、时钟回退、回归覆盖限制 | 主体分工已好；精简末尾首次连通回顾 |
...
| FOC | [无感接管](https://github.com/Awes0meE/Awes0meE.github.io/blob/ce2411e/content/notes/sensorless-foc-handoff.mdx) | 四层波形、坐标变换、SVPWM/TIM1、速度与角度、接管条件 | 保留技术主体，精简首尾经历与远期目标 |
