...
- `2026-08-12`: Git branch naming was standardized to `<semantic-prefix>/<kebab-case-purpose>` with Conventional Commits-aligned prefixes and no author/tool identity prefix. The Windows documentation cleanup moved from `agent/neat-windows-sync` to `docs/neat-windows-sync`; pull request #6 was closed and replaced by pull request #7 with the same original documentation commit. The fully merged `build/upgrade-next-16` and `agent/rewrite-arduino-smart-car-content` branches were removed locally or remotely as applicable.
