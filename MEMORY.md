...
- Continue portfolio rewrites one project family at a time; use `docs/active-work/portfolio-copy-rewrite.md` for the current selection and interview state, and do not bulk-rewrite the existing 21 published notes. Do not reopen the approved Smart Car, DIY Cooling, Arduino Digital Clock, Tianjin STM32, or Nanjing Turing families unless the user explicitly requests it.
...
- Use `engineering-note-writer` for future "turn my rough project evidence into portfolio prose" tasks. The approved sequence is: inspect artifacts; reuse a user-confirmed `grill-me` brief or obtain one; research beyond uploaded material with current reliable sources after confirmation; draft Chinese through the user's cognition trail; adapt English with the same substance and calmer intensity; pass both the Truth gate and Li Zhiyi gate. Evidence bounds project facts, authorship, actions, results, measurements, uncertainty, and publication claims but cannot choose reader-facing structure. External knowledge may supply light nearby links and present-learning insight, never invented project history. Cognition may move across subsystems or chronology and synthesize later when genuine, without a mandatory template; specifically requested neutral audits, summaries, and captions remain neutral and use the gates within that scope.
...
Known-good checks as of `2026-08-13` on macOS; use the equivalent `npm.cmd` form on Windows PowerShell:
...
- the `engineering-note-writer` skill validator passes; its final 34-file SHA-256 manifest matches; retained Trials 06-11 share runtime commit `9eab429` and pass every applicable GREEN gate.
