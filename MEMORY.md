...
- Use `engineering-note-writer` for future "turn my rough project evidence into portfolio prose" tasks. Evidence, authorship, uncertainty, and public claims are governed by `references/evidence-and-boundaries.md`. Let the material determine the article's order instead of selecting a named note archetype or fixed section sequence; derive specific headings after the argument is clear and keep them at 15 or fewer. Draft Chinese first, then write an English counterpart with the same substance. Calibrate Chinese toward roughly 75% and English toward roughly 60% of the Khazix Writer reference's emotion, wit, and personal presence; these are qualitative direction markers, not sentence-level quotas. Preserve concrete tools, files, symptoms, constraints, measurements, and unfinished edges, and do not invent metadata, paths, authorship, results, or certainty.
...
- `skills/engineering-note-writer/references/material-led-composition.md`: material-led flow, content-specific headings, transitions, and endings without fixed note archetypes.
