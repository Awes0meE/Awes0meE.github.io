...
| G6 | Chinese sentence gate | After Chinese editorial refinement and before English at Step 10, all six categories in `chinese-sentence-gate.md` have been reviewed and no established fault remains. The final Chinese is checked again at Step 12. Repairs preserve meaning and voice; the gate does not apply its six categories to English, and English-only work is not applicable. |
...
For a first-person learning-note trial produced under the current runtime, any applicable G1-G6 failure makes the whole trial fail. Factual safety cannot compensate for missing voice, an L1 violation, a Chinese sentence fault, lost source material, or an imported persona. Record `not applicable` for a move unsupported by the source or the Chinese sentence gate in English-only work; never manufacture missing material. For other requested forms, judge whether the output matches that form rather than falsely applying the learning-note voice gate.
