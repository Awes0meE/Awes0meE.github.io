...
| D10 | A note rewrite removes a repeated handoff story that remains on the protected homepage, linking to it in both languages. A measurement found only in the note is still relevant to its conclusion. | The handoff reduction is allowed within the note scope; conserve the measurement and its conditions. Do not restore every duplicate under editorial conservation or delete a unique fact under deduplication. |
| D11 | A unique confirmed event does not fit the proposed note focus and would require adding material to a protected homepage to preserve it. | Keep it within scope or raise that specific scope decision. Do not silently edit the homepage, erase the event, or treat an inaccessible historical snapshot as an adequate reader destination. |
