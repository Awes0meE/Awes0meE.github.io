...
## Use The Approved Spoken Palette Naturally

Use these as optional moves, never as a quota or a checklist:
...
Use the strong-reaction and profanity groups only when the confirmed source voice supports that intensity. A confirmed emotion may be phrased naturally around a confirmed real event, but do not invent the surrounding facts. Keep `首先`, `其次`, and `最后` available when ordinary prose genuinely needs them; only repeated sequence scaffolding is a smell.
