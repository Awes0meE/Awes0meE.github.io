...
From the x64 Native Tools Command Prompt, I verified `cl`, `link`, `nmake /?`, and `qmake -v`.
...
The Release executable alone was not enough. I copied only `seamly2d.exe` into a fresh folder such as `C:\Users\xxx\Desktop\Seamly2D_Release`, then ran `windeployqt` from the x64 Native Tools Command Prompt using the absolute Qt 6.5.3 `windeployqt.exe` path.

`windeployqt` copied the Qt runtime dependencies, but it did not copy the third-party XML library used by Seamly2D. The missing DLL was `xerces-c_3_2.dll`, from Apache Xerces-C++. Because Seamly2D stores pattern and measurement data in XML formats, this dependency is expected. I copied it manually from the source tree's `src\libs\xerces-c\msvc\lib` folder.
...
The two important outputs were the local root certificate `MyPersonalRootCA.cer` and the password-protected signing certificate `MySoftware.pfx` under `C:\MyCertificates`.
...
The installer itself was then signed manually with Windows SDK `signtool.exe`, a timestamp server, SHA256 digest options, the `.pfx` certificate, and the installer path. A successful `Successfully signed` message meant the installer was signed and ready for distribution in this learning context.
