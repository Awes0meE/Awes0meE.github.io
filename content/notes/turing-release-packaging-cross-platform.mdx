...
summary: "The original Seamly2D release packaging Markdown rendered as a full webpage note, covering Windows dependency collection, signing, Inno Setup, and follow-up packaging checks."
summaryZh: "把 Seamly2D Release 打包发布流程的原始 Markdown 直接渲染成完整网页笔记，覆盖 Windows 依赖收集、签名、Inno Setup 和后续验证。"
date: "2025-08-25"
tags: ["Qt", "Release", "windeployqt", "macdeployqt", "Packaging"]
visibility: public
projectSlug: "nanjing-turing-qt-embedded-learning"
---
<!-- turing-english-translation: release-packaging -->

## English Translation / 英文译文

### Source

The text below is the original `seamly2d-release-packaging-flow.md` workflow note. It records the complete Release path after the project could already build and run in Debug: environment checks, Windows dependency collection, code signing, Inno Setup packaging, verification, and follow-up macOS packaging work.

- [Open original Markdown](/uploads/projects/nanjing-turing/seamly2d-release-packaging-flow.md)
- [Open PDF export](/uploads/projects/nanjing-turing/seamly2d-release-packaging-flow.pdf)
- [macOS packaging script](/uploads/projects/nanjing-turing/macos-build-package.sh)
- [macOS signing-check script](/uploads/projects/nanjing-turing/macos-build-package-sign.sh)

### Seamly2D Release Packaging And Distribution Flow

This note records how I packaged a second-developed version of the open-source Seamly2D project from a working build into a distributable installer. It extends the earlier Windows 11 Qt6 setup note: after Debug builds were running and the added features were complete, the next task was to produce a Release build, collect runtime dependencies, sign the binaries, and create an installer.

### Pre-Release Environment Check

Before packaging, I checked that Visual Studio Build Tools still contained the required Windows SDK and `MSVC v142 - VS 2019 C++ x64/x86 build tools`. I also checked Qt Maintenance Tool to confirm that Qt 6.5.3 contained `MSVC 2019 64-bit`, the Qt 5 Compatibility Module, Qt Shader Tools, Qt Multimedia, Qt Creator, CDB Debugger Support, and Debugging Tools for Windows.

The system `Path` needed normal base tools such as Windows system paths, PowerShell, OpenSSH, Git, CMake, and Qt Creator. MSVC tools such as `cl`, `link`, and `nmake` were not expected to live directly in global PATH; Visual Studio normally configures them through its developer command prompts.

From the x64 Native Tools Command Prompt, I verified:
...
In Qt Creator, I opened `Seamly2D.pro`, cloned the existing Debug kit to avoid disturbing it, and checked that the cloned kit used `MSVC 2019 v142 (BuildTools)`, auto-detected CDB, and Qt 6.5.3 MSVC2019 64-bit. Then I added a Release build configuration. After a long compile, Qt Creator produced `seamly2d.exe` under the Release build output, and launching it directly confirmed that the app still worked.

### Collecting Windows Dependencies With windeployqt

The Release executable alone was not enough. I copied only `seamly2d.exe` into a fresh folder such as `C:\Users\xxx\Desktop\Seamly2D_Release`, then ran `windeployqt` from the x64 Native Tools Command Prompt using an absolute Qt path:
...
`windeployqt` copied the Qt runtime dependencies, but it did not copy the third-party XML library used by Seamly2D. The missing DLL was `xerces-c_3_2.dll`, from Apache Xerces-C++. Because Seamly2D stores pattern and measurement data in XML formats, this dependency is expected. I copied it manually from:

```text
{Seamly2D source path}\src\libs\xerces-c\msvc\lib
```

After copying the DLL into the release folder, double-clicking `seamly2d.exe` should run the program with the same developed features.

### Windows Code-Signing Certificate

The note then explains why signing matters. Unsigned software often triggers Windows SmartScreen warnings, macOS developer-verification prompts, and antivirus false positives. Commercial software should normally use an OV or EV code-signing certificate from a trusted CA such as DigiCert, Sectigo, or GlobalSign. EV certificates provide stronger verification and can build SmartScreen reputation faster, but they are more expensive and often require hardware token storage.

For this personal/small-scale task, I used a self-signed certificate as a zero-cost learning path. It does not create the same public trust as a commercial certificate, but it lets the workflow exercise certificate creation, signing, and installation.

The Windows self-signed process was:

1. Run PowerShell as administrator.
2. Create a local root CA certificate with `New-SelfSignedCertificate`.
3. Export the root certificate as `MyPersonalRootCA.cer` so it can be installed on user machines.
4. Create a code-signing certificate signed by that root certificate.
5. Export the signing certificate as a password-protected `.pfx` file.

The two important outputs were:

```text
C:\MyCertificates\MyPersonalRootCA.cer
C:\MyCertificates\MySoftware.pfx
```

The `.cer` file is distributed so the user can trust the local root certificate. The `.pfx` file stays with the developer and is used to sign the application.

### Creating The Installer With Inno Setup

Inno Setup was used to create a normal Windows installer. The expected user experience is a familiar wizard: next, choose installation directory, optionally create shortcuts, and install the application under Program Files.

After installing Inno Setup from `https://jrsoftware.org/isdl.php`, I created an empty script. The script defined the application name, version, publisher, URL, executable name, signing tool path, certificate path, and PFX password. In `[Setup]`, it set `AppId`, `AppName`, `AppVersion`, default installation directory, output filename, compression, and related metadata. In `[Files]`, it included all files from the `Seamly2D_Release` folder and also included `MyPersonalRootCA.cer`. In `[Icons]`, it created Start Menu and optional desktop shortcuts. In `[Run]`, it installed the root certificate for the current user with `certutil`.

The script also included `[Code]` logic to run `signtool.exe` after installation and sign the main executable and DLLs. After compiling the `.iss` script, Inno Setup produced `Seamly2D_Setup.exe` under the `Output` folder.

The installer itself was then signed manually with:

```powershell
"C:\Program Files (x86)\Windows Kits\10\bin\10.0.19041.0\x86\signtool.exe" sign /tr http://timestamp.digicert.com /td SHA256 /fd SHA256 /f "C:\MyCertificates\MySoftware.pfx" /p "SecurePassword123" "[absolute path]\Seamly2D_Setup.exe"
```

A successful `Successfully signed` message meant the installer was signed and ready for distribution in this learning context.

### Verification And Notes

The signature can be checked from file properties under the Digital Signatures tab. If a signature entry exists and the detail view shows the certificate chain has been processed, the signing step worked. Because the certificate is self-signed, the issuer may still be shown as untrusted until the root certificate is installed.

For installation verification, I ran `Seamly2D_Setup.exe`, followed the wizard, checked that the root certificate was installed, found `seamly2d.exe` under the installed directory, confirmed that the executable had a digital signature, and launched the program successfully.

Ideally, the final installer should also be tested on a clean Windows machine without Visual Studio or Qt Creator installed. I did not have another Windows device at hand, so this note records the tested local path: build, dependency collection, certificate injection, installation, and launch all worked on my machine.

The macOS part followed the same release idea: move the developed code to a Mac, resolve Qt path and architecture differences, build Debug first, switch to Release, run `macdeployqt` to collect dependencies into the `.app`, and experiment with signing. The helper shell scripts keep build, packaging, dependency collection, and signing checks closer to one repeatable workflow. The complete formal Apple signing/notarization path was not fully finished, but the non-signing packaging flow was largely connected.

## 原文来源

下面直接放 `seamly2d-release-packaging-flow.md` 的正文。它本来就是一篇完整流程记录，所以网页里保留原来的步骤、命令、解释和后记。

- [打开原始 Markdown](/uploads/projects/nanjing-turing/seamly2d-release-packaging-flow.md)
- [打开 PDF 导出](/uploads/projects/nanjing-turing/seamly2d-release-packaging-flow.pdf)
- [macOS 打包脚本](/uploads/projects/nanjing-turing/macos-build-package.sh)
- [macOS 签名检查脚本](/uploads/projects/nanjing-turing/macos-build-package-sign.sh)
...
>
>
> [Windows11从0开始在Qt6环境下跑通Seamly2D的全流程记录.docx](/uploads/projects/nanjing-turing/windows11-qt6-seamly2d-onboarding.docx)
>
