...
Alongside the output calculation, `updatePWM()` contains two assignments that clear the peaks. The excerpt below includes both operations.

`updatePWM()` 在计算输出之外，还会清空峰值。下面摘出函数中计算输出和清空峰值的部分。
