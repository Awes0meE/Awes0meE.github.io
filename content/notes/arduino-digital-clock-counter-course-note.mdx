...
## Current in Each Segment / 每一段的电流
...
## Matching the Decoder and Display / 译码器的配合
...
回到最初的 `17`，我现在会分别想到十位和个位的 BCD、被选中的段，以及经过每只 LED 的电流。若某一段始终不亮，可以沿那条支路找；若整位稳定显示成另一个数，就先对照输入位序和译码条件。一个小小的显示结果，终于能和具体引脚、具体电流对应起来了。以后再碰到数码管，我也更知道该从哪里开始看。
